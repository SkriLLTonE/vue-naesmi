  ...
  <div style="width: 100%; display: flex; justify-content: center" ref="main_wrapper">
    <div style="width: 100%; max-width: 1200px">
      ...
      <transition name="fade" appear>
        <div style="margin-top: 50px" class="lds-spinner" v-if="news.length === 0 && !empty">
          <div></div>
          <div></div>
          <div></div>
          <div></div>
      ...
      <transition name="fade" appear>
        <span style="font-size: 50px; padding: 40px 0" v-show="empty.length !== 0">{{empty}}</span>
      </transition>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getNews() {
      this.empty = ""
      fetch(
              `http://127.0.0.1:8000/news2/?search=${this.query}&limit=4&offset=${
                      this.offset * 4
              }`
      )
              .then((res) => {
              ...
              .then((json) => {
                ...
                if (json.count === 0) this.empty = "Hech narsa topilmadi"
                this.offset++;
                console.log(json);
